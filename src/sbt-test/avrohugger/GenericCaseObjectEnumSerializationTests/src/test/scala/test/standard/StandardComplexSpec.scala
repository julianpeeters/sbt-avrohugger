...
Avro4s is used to convert to `GenericRecord` for testing, chokes on `Map`
  ...
  */
  /* Enum test uses GenericRecord API directly since avro4s won't do companions
  ...
  "A case class with an enumeration field" should {
    "serialize and deserialize correctly" in {
      val schema = com.sksamuel.avro4s.AvroSchema[Compass]
      val record1 = new org.apache.avro.generic.GenericData.Record(schema)
      record1.put("direction", Direction.NORTH);
      val record2 = new org.apache.avro.generic.GenericData.Record(schema)
      record2.put("direction", Direction.SOUTH);
